...
void autonomous(){
 ...
 //     {5, 8}, // Left motors are 1 & 2 (reversed)
	...
	profileController->generatePath({
  	...
  	{100_in, -120_in, 0_deg}}, // The next point in the profile, 3 feet forward
  	"A" // Profile name
	);
	...
	straight_line_drive(52.0, 'n', 85, 500, 5000);
	profileController->setTarget("A", true);
	profileController->waitUntilSettled();
	straight_line_drive(55.0, 0, 80, 500, 5000);
	pros::delay(5000);
	tilter::setTarget(tilter::State_Machine::E_STACK);
	if(tilter::isDone == true){
		drive_outtake(-100, -25, 1000);
	}
