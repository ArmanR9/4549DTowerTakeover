...
const float TRAY_FORWARD_VAL = -4900;
  ...
  while(true) {
    while (anglerBool) {
      ...
      if (anglerIntakeThreshold || (currentTarget == TRAY_BACKWARD_VAL)) {
        ...
        if (pros::c::motor_get_torque(12) > maxTorque) {
          maxTorque = pros::c::motor_get_torque(12);
        }
        ...
        float position = tilter_mtr.get_position();
        int final_power = pid_calc(&angler_pid, currentTarget, position);
        tilter_mtr.move(final_power);
