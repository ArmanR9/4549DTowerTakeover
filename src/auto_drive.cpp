...
void driveToPosition(float y, float x, float ys, float xs, float maxErrX, bool enableCorrect, bool forward){

// Initialize PID
  PID pos_drive(0.5, 0.0, 0.0, 2.0, 100, 200, 5000);
...
// Path line
  ...
  pos_drive.calculateFailsafe(5000);
  ...
  float final_power;
   ...
   _Dir direction = forward == true ? _Dir::FWD : _Dir::BWD;
      ...
      while(pos_drive.getTimer() > pros::millis() && pos_drive.getFailsafe() > pros::millis()){
